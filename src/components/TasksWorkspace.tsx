...
import { Columns3, List, RefreshCw, Users, X } from "lucide-react";
...
export function TasksWorkspace({
  ...
  // Extract unique filter options from tasks
  const allDomains = [...new Set(tasks.filter((t) => t.domainName).map((t) => t.domainName!))].sort();
  const allProjects = [...new Set(tasks.filter((t) => t.projectTitle).map((t) => t.projectTitle!))].sort();

  const filtered = tasks.filter((t) =>
    (!filterDomain || t.domainName === filterDomain) &&
    (!filterProject || t.projectTitle === filterProject) &&
    (!filterStatus || t.status === filterStatus)
  );

  const hasFilter = filterDomain || filterProject || filterStatus;
  ...
  return (
    <div className="animate-fade-in">
      ...
      {/* Filter bar */}
      {(allDomains.length > 0 || allProjects.length > 0) && (
        <div className="mb-4 flex flex-wrap items-center gap-1.5">
          ...
          {allDomains.map((d) => (
            <button key={d} onClick={() => setFilterDomain(filterDomain === d ? null : d)}
              ...
              style={{ background: filterDomain === d ? "var(--accent)" : "transparent", color: filterDomain === d ? "#fff" : "var(--muted)", borderColor: filterDomain === d ? "var(--accent)" : "var(--border)" }}>
              {d}
            </button>
          ))}
          {allProjects.map((p) => (
            <button key={p} onClick={() => setFilterProject(filterProject === p ? null : p)}
              className="rounded-full border px-2.5 py-0.5 text-xs transition-colors"
              style={{ background: filterProject === p ? "var(--ok)" : "transparent", color: filterProject === p ? "#fff" : "var(--muted)", borderColor: filterProject === p ? "var(--ok)" : "var(--border)" }}>
              {p}
            </button>
          ))}
          {STATUSES.map((s) => (
            <button key={s} onClick={() => setFilterStatus(filterStatus === s ? null : s)}
              className="rounded-full border px-2.5 py-0.5 text-xs transition-colors"
              style={{ background: filterStatus === s ? STATUS_META[s].color : "transparent", color: filterStatus === s ? "#fff" : "var(--muted)", borderColor: filterStatus === s ? STATUS_META[s].color : "var(--border)" }}>
              {STATUS_META[s].label}
            </button>
          ))}
          {hasFilter && (
            <button onClick={() => { setFilterDomain(null); setFilterProject(null); setFilterStatus(null); }}
              className="inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs text-muted hover:text-danger">
              <X size={11} /> Clear
            </button>
          )}
        </div>
      ...
      {view === "list" && (
        <div className="space-y-5">
          ...
          {filtered.length === 0 && hasFilter && (
            <div className="rounded-xl border border-dashed p-6 text-center text-sm text-faint">No tasks match the active filters.</div>
          )}
        </div>
      )}
      ...
      {view === "team" && (
        <div className="space-y-4">
          {[...members.map((m) => ({ id: m.id, name: m.name, color: m.color })), { id: "__none__", name: "Unassigned", color: "#94a3b8" }].map((m) => {
            const group = filtered.filter((t) => (m.id === "__none__" ? !t.responsibleId : t.responsibleId === m.id));
            if (group.length === 0) return null;
            return (
...
function MetaRow({ t, member }: { t: PlannerTask; member?: Member }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 text-xs text-muted">
      ...
      {t.subtaskCount > 0 && (
        <span className="text-faint">{t.subtaskDone}/{t.subtaskCount} ✓</span>
      )}
    </div>
  );
}
...
function Row({ t, member, onClick, compact }: { t: PlannerTask; member?: Member; onClick: () => void; compact?: boolean }) {
  ...
  return (
    <div
      ...
      className="flex w-full cursor-pointer items-center gap-3 rounded-xl border bg-surface p-2.5 text-left shadow-xs transition-shadow hover:shadow-sm"
    >
      <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: STATUS_META[t.status as Status]?.color }} />
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          <span className="truncate text-sm font-medium" style={{ textDecoration: done ? "line-through" : "none", color: done ? "var(--faint)" : "var(--ink)" }}>{t.title}</span>
          {t.recurrence && (
            <span title={`Repeats ${t.recurrence}`} className="shrink-0 text-faint">
              <RefreshCw size={11} />
            </span>
          )}
        </div>
      ...
      <div onClick={(e) => e.stopPropagation()}>
        <TaskTimerButton taskId={t.id} trackedSeconds={t.trackedSeconds} timerStartedAt={t.timerStartedAt} />
      </div>
    </div>
  );
}
...
function Card({ t, member, onClick, onDragStart }: { t: PlannerTask; member?: Member; onClick: () => void; onDragStart: () => void }) {
  return (
    <div draggable onDragStart={(e) => { e.dataTransfer.setData("text/plain", t.id); onDragStart(); }} onClick={onClick} className="cursor-pointer rounded-lg border bg-surface p-2.5 shadow-xs transition-shadow hover:shadow-md">
      <div className="flex items-center gap-1.5 text-sm font-medium" style={{ textDecoration: t.status === "completed" ? "line-through" : "none" }}>
        <span className="flex-1">{t.title}</span>
        {t.recurrence && <RefreshCw size={11} className="shrink-0 text-faint" />}
      </div>
